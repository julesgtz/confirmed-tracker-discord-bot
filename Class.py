...
class Tracker:
    ...
    def __recaptcha(self):
        response = self.s.get(
            "https://www.google.com/recaptcha/enterprise/anchor?ar=1&k=6LdQquAaAAAAALeU6cp88M5ByhWDANC1-ei8xfMW&co=aHR0cHM6Ly93d3cuYWRpZGFzLmZyOjQ0Mw..&hl=fr&v=iRvKkcsnpNcOYYwhqaQxPITz&size=invisible&cb=apiluerxij9t")
    ...
    def __get(self):
        ...
        r = self.s.post("https://www.adidas.fr/api/orders/search", headers=self.headers, data=payload)
        id = r.content.decode("utf-8")[2:-1].split('"')[2].replace("/", "%2F")
        info = self.s.get(f"https://www.adidas.fr/api/orders/{id}", headers=self.headers)
    ...
    def get_data(self):
        ...
        if info == None:
            ...
            try:
                invoice = info.json().get("invoiceListId")
                rq = self.s.get(
                    "https://www.adidas.fr/api/orders/invoice/list/" + invoice.replace("+", "%2B").replace("/","%2F").replace("=", "%3D"), headers=self.headers)
                if rq.ok:
                    invoice = "https://www.adidas.fr/api/orders/invoice/" + rq.json()[0].get("id").replace("+","%2B").replace("/", "%2F").replace("=", "%3D")
